    ...
    Protected Sub Page_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        ...
        Else
            ...
            If Not IsPostBack Then
                txtid.Text = HttpUtility.UrlDecode(Request.QueryString("idunit"))
                ...
                Using con As New SqlConnection(constr)
                    Using cmd As New SqlCommand("select idcabang as Kampus from TblCabang order by idcabang Asc")
                        cmd.CommandType = CommandType.Text
                        cmd.Connection = con
                        con.Open()
                        ddlunit.DataSource = cmd.ExecuteReader()
                        ddlunit.DataTextField = "Kampus"
                        ddlunit.DataValueField = "Kampus"
                        ddlunit.DataBind()
                        con.Close()
                    End Using
                End Using
                ddlunit.Items.Insert(0, New System.Web.UI.WebControls.ListItem("Pilih Cabang...", ""))
...
#Region "Button"
    ...
    Protected Sub lnkSave_Click(sender As Object, e As System.EventArgs) Handles lnkSave.Click
        ...
        ElseIf String.IsNullOrEmpty(txtlokasi.Text.Trim()) Then
            ...
            Exit Sub
        End If
        Dim cabangunit As String = Left(ddlunit.Text, 2)
        'Check untuk melakukan tambah data
        If Not CheckAdaData(txtid.Text.Trim(), "idunit", "TblUnitKerja") Then
            connectDB()
            mySqlCon.Open()
            ...
            Try
                mySqlCmd.CommandText = "INSERT INTO TblUnitKerja (idCabang,idunit, lokasi) VALUES ('" & cabangunit & "' ,'" _
        ...
        Else
            ...
            Try
                mySqlCmd.CommandText = "UPDATE TblUnitKerja SET idcabang='" & cabangunit & "',lokasi = '" & txtlokasi.Text.Trim() & "' WHERE idunit = '" & txtid.Text.Trim() & "'; "
                mySqlCmd.ExecuteNonQuery()
...
#End Region
    ...
    Private Sub QueryAccount(ByVal a_Str As String, showmsg As Boolean)
        ...
        Finally
            ...
            command = New System.Data.OleDb.OleDbCommand("SELECT idcabang as idcabang, idunit as idunit,lokasi as lokasi FROM TblUnitKerja WHERE idunit = '" & a_Str & "'; ", mySqlCon)
            mySqlReader = command.ExecuteReader
            If mySqlReader.Read Then
                ddlunit.Text = mySqlReader("idcabang").ToString()
                txtid.Text = mySqlReader("idUnit").ToString()
...
#End Region
    ...
    Public Sub NOOTOMATIS()
        Dim NO As String = clsEnt.ReturnOneFieldConn("select top(1)idunit + 1 as nnn from TblUnitKerja order by idunit desc ", "", "", "nnn", "connuq")
    ...
    Protected Sub lnkDelete_Click(sender As Object, e As System.EventArgs) Handles lnkDelete.Click
        ...
        Try
            mySqlCmd.CommandText = "DELETE FROM TblUnitKerja WHERE idunit = '" & txtid.Text.Trim() & "'; "
            mySqlCmd.ExecuteNonQuery()
