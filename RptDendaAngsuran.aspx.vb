...
Partial Class RptDendaAngsuran
...
#End Region
    ...
    Protected Sub lnkPrintJadwal_Click(sender As Object, e As System.EventArgs) Handles lnkPrintJadwal.Click
        ...
        If dtHead.Rows.Count >= 1 Then
            ...
            Dim filepath As String = Server.MapPath("PDF/RealisasiDenda" & Format(Now, "dd-MM-yyyy") & ".pdf")
            Using fs As New FileStream(Server.MapPath("PDF/RealisasiDenda" & Format(Now, "dd-MM-yyyy") & ".pdf"), FileMode.Create)
                ...
                doc.AddTitle("Daftar Realisasi Denda Nasabah ")
                doc.Open()
                ...
                Dim akmp3 As String = "REALISASI DENDA NASABAH"
                ...
                For x = 0 To dtRuang.Rows.Count - 1
                    ...
                    phrase = New Phrase("SALDO AWAL DENDA ", fntArial10N) : cell = PhraseCell(phrase, PdfPCell.ALIGN_CENTER, iTextSharp.text.Color.BLACK) : myTableCol.AddCell(cell)
                    phrase = New Phrase("REALISASI  ", fntArial10N) : cell = PhraseCell(phrase, PdfPCell.ALIGN_CENTER, iTextSharp.text.Color.BLACK) : myTableCol.AddCell(cell)
                    ...
                    For y = 0 To dtHariJadwal.Rows.Count - 1
                        dtHead = GetDataTbl("SELECT CONVERT(VARCHAR(10),a.Tanggal,103) as tanggal,isnull(a.idTrans,'') as nopinjaman,isnull(b.nama,'') as Nama,saldoawal=(Select sum(e.denda) from TblAngsuran e where e.NoPinjaman=a.idTrans and e.wipem='" & dtRuang.Rows(x).Item("idwipem") & "' and e.idUnit='" & dtRuang.Rows(x).Item("idUnit") & "'),realisasi=(Select sum(e.jumlah) from Tbldenda e where e.NoPinjaman=a.idTrans and e.wipem='" & dtRuang.Rows(x).Item("idwipem") & "' and e.idUnit='" & dtRuang.Rows(x).Item("idUnit") & "'),Saldoakhir=(Select Sum(c.denda)  from TblAngsuran c where c.nopinjaman=a.idTrans And c.wipem='" & dtRuang.Rows(x).Item("idwipem") & "' and c.idUnit='" & dtRuang.Rows(x).Item("idUnit") & "') - (Select sum(d.jumlah) from TblDenda d where d.NoPinjaman=a.idTrans And d.wipem='" & dtRuang.Rows(x).Item("idwipem") & "' and d.idUnit='" & dtRuang.Rows(x).Item("idUnit") & "')  from Tbljual  a inner join tblnasabah b on b.idnasabah=a.idnasabah  where  a.wipem='" & dtRuang.Rows(x).Item("idwipem") & "' and a.idUnit='" & dtRuang.Rows(x).Item("idUnit") & "'  order by a.idPrimary asc;")
